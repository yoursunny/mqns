...
def es_rule_ASAPactionB(memories_info: List["MemoryInfo"], args: Arguments):
    ...
    memories = [info.memory for info in memories_info]
    memory = memories[0]
    #DEBprint("Action applying B in ", memory.name)
    protocol = EntanglementSwappingB(None, "ESB." + memory.name, memory)
    #### expire the rule
    node = args["node"]
    rule = args["rule"]
...
def es_rule_conditionB1(memory_info: "MemoryInfo", manager: "MemoryManager", args: Arguments):
    ...
    if (memory_info.state == "ENTANGLED"
            ...
            # and memory_info.fidelity >= reservation.fidelity):
            and memory_info.fidelity >= fidelity):
        #DEBprint(f"{manager.resource_manager.owner.timeline.now()} will apply B in {memory_info.memory.owner.name}")
        return [memory_info]
    else:
        return []
...
def es_rule_ASAP_conditionB1(memory_info: "MemoryInfo", manager: "MemoryManager", args: Arguments):
    """Condition function used by EntanglementSwappingB protocol on nodes of either responder or initiator
    """
    #DEBprint("will verify B for ", memory_info.memory.name)
    ...
    #DEBprint(f"{memory_info.state} {memory_info.remote_node}")
    if (memory_info.state == "ENTANGLED"
        and memory_info.memory.name == desired_memory
        ):
...
def load_B_rules(left_node: "QuantumRouter", left_node_memory: "str", right_node: "QuantumRouter", right_node_memory: "str"):
    #DEBprint("will try to load B between ", left_node.name, right_node.name)
    ...
    rule.action_args = {"node" : left_node,
                   "rule": rule}
    left_memory_info = next((m for m in left_node.resource_manager.memory_manager.memory_map if m.memory.name == left_node_memory), None)
    ...
    action_args ={}
    rule = Rule(5, es_rule_ASAPactionB, es_rule_ASAP_conditionB1, action_args, condition_args)
    rule.action_args = {"node" : right_node,
                   "rule": rule}
    right_memory_info = next((m for m in right_node.resource_manager.memory_manager.memory_map if m.memory.name == right_node_memory), None)
...
def loadB(left_node: "QuantumRouter", left_node_memory: "str", right_node: "QuantumRouter", right_node_memory:"str"):
    #DEBprint("will try to load B between ", left_node_memory, right_node_memory)
    for memo in left_node.get_components_by_type("MemoryArray")[0]:
        ...
        if memo.name == left_node_memory:
            ...
            #DEBprint("B done in ", left_node_memory)
    for memo in right_node.get_components_by_type("MemoryArray")[0]:
        #DEBprint(memo.name,right_node_memory)
        if memo.name == right_node_memory:
            ...
            #DEBprint("B done in ", right_node_memory)

def es_rule_ASAP_conditionA(memory_info: "MemoryInfo", manager: "MemoryManager", args: Arguments):
    """Condition function used by EntanglementSwappingA protocol on nodes
    """
    #DEBprint(f"{manager.resource_manager.owner.timeline.now()}will verify A in {memory_info.memory.name} : {memory_info.state}")
    memory_indices = args["memory_indices"]
    fidelity = args["fidelity"]
    path = args["path"]
    index = args["index"] 
    ...
    if (memory_info.state == "ENTANGLED"
            ...
            and memory_info.fidelity >= fidelity):
        for info in manager:
            #DEBprint(manager.resource_manager.owner.timeline.now(), info.memory.name,info.state, info.remote_node)
            if (info.state == "ENTANGLED"
                    and info.index in memory_indices
                ...
                #DEBprint(f"{manager.resource_manager.owner.timeline.now()} will apply A in {memory_info.memory.owner.name} between {memory_info.remote_node} . {memory_info.memory.name} AND {info.remote_node} . {info.memory.name}")
                return [memory_info, info]
    elif (memory_info.state == "ENTANGLED"
          and memory_info.index in memory_indices
        ...
        for info in manager:
            ...
            if (info.state == "ENTANGLED"
                ...
                #import time;time.sleep(0.001)
                #DEBprint(f"{manager.resource_manager.owner.timeline.now()} will apply A in {memory_info.memory.owner.name} between {info.remote_node} . {info.memory.name} AND {memory_info.remote_node} . {memory_info.memory.name}")
                return [memory_info, info]
    return []
...
def es_rule_ASAP_conditionB2(memory_info: "MemoryInfo", manager: "MemoryManager", args: Arguments) -> List["MemoryInfo"]:
    """Condition function used by EntanglementSwappingB protocol on intermediate nodes of path
    """
    #DEBprint("will validate B")
    memory_indices = args["memory_indices"]
    fidelity = args["fidelity"]
    if memory_info.state == "ENTANGLED":print("B ",memory_info.state)  
    if (memory_info.state == "ENTANGLED"
            and memory_info.index in memory_indices
            and memory_info.fidelity >= fidelity):
        #DEBprint(f"{manager.resource_manager.owner.timeline.now()} will apply B in {memory_info.memory.owner.name}")
...
def create_rules(self, path: List[str], reservation: "Reservation") -> List["Rule"]:
    ...
    if reservation.link_capacity:
        ...
        if index < len(path) - 1:
            if index == 0:
                ...
                condition_args = {"memory_indices":
                                        memory_indices[reservation.link_capacity:reservation.link_capacity + reservation.link_capacity]}

            action_args = {"mid": self.owner.map_to_middle_node[path[index + 1]],
                            "path": path, "index": index, "name": self.owner.name,
        ...
        if index > 0:
            condition_args = {"memory_indices":
                                    memory_indices[:reservation.link_capacity],
                                "reservation": reservation}
            action_args = {}
        ...
        if index < len(path) - 1:
            if index == 0:
                ...
                condition_args = {"memory_indices": memory_indices[reservation.link_capacity:reservation.link_capacity+reservation.link_capacity],
                                    "fidelity": reservation.fidelity}
