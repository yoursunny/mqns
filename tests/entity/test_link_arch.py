...
def test_dim_bk():
    ...
    check_delays(link_arch, attempt_duration=(2, 2), notify_a=(2, 2), notify_b=(2, 2))


def test_dim_bk_seq():
    link_arch = LinkArchDimBkSeq()
    check_delays(link_arch, attempt_duration=(5, 2), notify_a=(4, 2), notify_b=(4, 2))


def test_dim_dual():
    link_arch = LinkArchDimDual()
    check_delays(link_arch, attempt_duration=(1, 1), notify_a=(1, 1), notify_b=(1, 1))


def test_sr():
    link_arch = LinkArchSr()
    check_delays(link_arch, attempt_duration=(2, 1), notify_a=(1, 1), notify_b=(2, 1))
