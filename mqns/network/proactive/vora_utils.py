# -*- coding: utf-8 -*-
"""
Created on Thu Jul 25 21:54:00 2024
...
def greedyswap(C, p=1, q=0.5, prnt=False, cutoff=1, Bq=None, Ts=1):
    ...
    if prnt: 
        print('Swapping order', swap_sq, 
              '  => E[ent.]: {:.6f}'.format(EXT), 
              '\ttime: {:.6f}'.format(runtime))
        
    return {'order': swap_sq, 
            'EXT': EXT, 
            'P': P, 
            'time': runtime}
...
def voraswap(C, p=1, q=0.5, prnt=False, 
             cutoff=.9995, Bq=None, 
             Ts=1, 
             names=''):
    ...
    if prnt: 
        ...
        print('\tgreedyswap = %.2f'%(greedy_heu['EXT']),
              '\truntime = %.3e (s)'%(greedy_heu['time']), 
              '\tSwapOrder:', s_tr(order2tree(greedy_heu['order']), names), 
             '\t <== HEU. SOL.' if greedy_heu['EXT'] >= baln_heu['EXT'] else '')
        
        print('\tbaln__tree = %.2f'%(baln_heu['EXT']), 
              '\truntime = %.3e (s)'%(baln_heu['time']), 
              '\tSwapOrder:', s_tr(tree, names), 
             '\t <== HEU. SOL.' if greedy_heu['EXT'] < baln_heu['EXT'] else '')
...
def net_chain3(L, p=None, q=None, C=None, M=None, 
               note='', unit=''):
    # Example usage:
    # L_values = [1, 2, 3, 4]
    ...
    if C is not None: 
        ...
        print('   C:   ',C_line)
    if p is not None: 
        if type(p) is int or type(p) is float: print(  'p:'+' '*8+'%.2f'%(p))
        else: 
            p_line = ' '*len(net)
    ...
    if q is not None: print(  '   q: '+' '*net.find(f'(R{1})'), q)
...
def plot_path(L_list, M=None, order=None, endnodes=['S', 'D']): 
    ...
    if order is not None: 
        ...
        for i,node in enumerate(order): nodes[node] = str(i+1)
