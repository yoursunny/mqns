...
class ClassicConnector:
    ...
    * Header ``fmt``: packet format, either ``"json"`` or ``"bytes"``
    ...
    This , which includes a clock gate feature
    , until the external program has
    processed past packets and scheduled all packets before the clock gate.
    ...
    * Header ``t``: new upper-bound in simulation time slots
    ...
    async def _rx_loop(self):
        from nats.js.errors import FetchTimeoutError  # noqa: PLC0415
    ...
    def _rx(self, dst: str, src: str, headers: dict[str, str], payload: bytes):
        if (dst, src) == ("_", "gate"):
            self.simulator.update_gate(self.simulator.time(time_slot=int(headers.get("t", 0))))
            return
        ...
        self.simulator.add_event(func_to_event(self.simulator.time(time_slot=cbp.t), bridge.inject, cbp, by=self))
